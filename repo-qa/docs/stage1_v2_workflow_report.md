...
## 18. 阶段总结（重构前）：已完成 / 未完成 / Stage1 完成度

- 已完成（可用性维度）：
  - 分解工具化（可调用、可追踪、可重分解）；
  - 图工具化（retrieve/validate）与工具调用记录；
  - 终端与轨迹可观测性（tool_calls/subquestion_trace）；
  - 宽扫描补偿策略（预算 + 重写 + 图引导）；
  - 外部数据接入（SWE-QA-Bench 抽取脚本，实验可直接消费）。
- 未完成（研究质量维度）：
  - 分解质量指标仍以代理指标为主，缺少人工标注/任务回报校准；
  - 主动图调度尚未做到“动作生成前硬约束规划器”；
  - 尚未形成系统化 RL reward 与 failure taxonomy 闭环。
- 结论：
  - 当前版本已达到“功能完善、可跑大规模实验”的工程可用状态；
  - 作为 Stage1（做一个好的问题分解工具）的目标，基础目标基本达成；
  - 若追求“好用且稳定到生产级”，仍需要后续重构与评估体系增强。
